...
remove_outliers <- function(df, ..., sd_thresh = 3) {
  ...
  # If no variables were given, return the original data with a warning
  if (length(vars_to_check) == 0) {
    warning("No variables specified for outlier removal. Returning original data.")
    return(df)
  }

  # Turn the captured column symbols into actual column names (as characters)
  ...
  # Warn the user if they referenced any columns that don’t exist
  if (length(non_existing_cols) > 0) {
    warning("These columns don’t exist and will be ignored: ",
            paste(non_existing_cols, collapse = ", "))
  }

  # Of the valid columns, figure out which ones are numeric
  ...
  # Warn if any of the selected columns are not numeric
  ...
  # If we have nothing numeric to check, return the original data
  ...
  # Filter the data to keep only rows where all specified numeric variables
    ...
    filter(
      if_all(
        ...
        ~ {
          ...
          if (is.na(s) || s == 0) {
            TRUE  # If no variation, don’t remove anything
          } else {
...
impute_missing <- function(df, ..., impute_fun = mean) {
  ...
  # If the user doesn’t pass any variables, just return the original data
  if (length(vars_to_impute) == 0) {
    warning("No variables specified for imputation. Returning original data.")
    return(df)
  }

  # Get the column names as character strings
  ...
  # Warn if any user-specified columns weren’t found
  if (length(non_existing_cols) > 0) {
    warning("These columns don’t exist in the data and will be skipped: ",
            paste(non_existing_cols, collapse = ", "))
  }

  # From the valid columns, keep only the numeric ones
  if (length(existing_selected_cols) > 0) {
    numeric_cols_to_process <- df %>%
  ...
  # Warn if any of the selected columns aren’t numeric
  non_numeric_selected <- setdiff(existing_selected_cols, numeric_cols_to_process)
  if (length(non_numeric_selected) > 0) {
  ...
  # If there’s nothing numeric to process, just return the original data
  if (length(numeric_cols_to_process) == 0) {
  ...
  # Go through each selected numeric column and fill in the missing values
  df_imputed <- df %>%
    mutate(
      across(
        all_of(numeric_cols_to_process),
        ~ {
          ...
          # If the whole column is NA, we can't impute anything — leave it as-is
          if (length(valid_values) == 0) {
            imputation_value <- NA
          } else {
            # Try applying the user-specified function to get an imputation value
            ...
            # Only keep the result if it’s a single atomic value (e.g., one number)
...
fit_model <- function(df, mod_formula, remove_outliers = FALSE, impute_missing = FALSE, ...) {
  # Grab the variables the user passed to ... (used for preprocessing)
  vars_for_processing <- rlang::enquos(...)

  # Work on a copy of the dataset so we don’t overwrite the original
  processed_df <- df

  # If user wants to remove outliers, run that step first
  if (remove_outliers) {
    # The remove_outliers() function will handle cases where variables are missing or not numeric
...
# Step 1: Set up the different model formulas we want to try
...
# We’ll test both options: with and without imputation
imputation_options <- c(TRUE, FALSE)

# Step 3: Decide whether or not to remove outliers
# Again, we’ll try it both ways (TRUE and FALSE)
...
# These column names match the argument names used in our fit_model() function
...
# Step 5: Take a look at the combinations we just created
...
# We're now going to apply fit_model() to each row of df_arg_combos
...
# Let's check out what we got — each row now includes the fitted model
print(df_results)
```
